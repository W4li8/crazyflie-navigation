...
class Pilot:
    ...
    def __init__(self, mission_file):
        ...
        self.takeoff_speed   = data["takeoff_speed"]
        ...
        self.last_waypoint = np.zeros(2)
        self.oscillation_threshold = 0.04 # 4 cm
        ...
        self.globalnav = GlobalNav(500,300,25,15,100,100,True)
        self.globalnav.radius = 1
        #! additional dictionary of flight settings we can add later
        print(data["mission_settings"])

    def takeoff(self):
        self.drone.hlcontroller.take_off(height=self.flight_altitude, velocity=self.takeoff_speed)
        time.sleep(1)
        self.drone.hlcontroller.forward(0.05, velocity=0.05)
        self.drone.hlcontroller.back(0.05, velocity=0.05)
        time.sleep(1)
    ...
    def waypoint(self,x,y):
        ...
        if (waypoint != self.last_waypoint).any():
            e_x = self.drone.get_pose()[0]
            e_y = self.drone.get_pose()[1]
            dx = x/100-e_x
            dy = y/100-e_y
            # self.drone.hlcontroller.forward(dx, velocity=self.cruise_speed)
            # self.drone.hlcontroller.left(dy, velocity=self.cruise_speed)
            if np.abs(dx) > self.oscillation_threshold or np.abs(dy) > self.oscillation_threshold:
                self.drone.hlcontroller.move_distance(dx,dy,0,velocity=self.cruise_speed)
            # tx = self.drone.logger.estimated_pose[0]+dx
            # ty = self.drone.logger.estimated_pose[1]+ dy
...
#         self.drone.hlcontroller.go_to(tx, ty, z=self.flight_altitude, velocity=self.cruise_speed)
#         # self.drone.hlcontroller.move_distance(dx, dy, 0, velocity=self.cruise_speed)
#
#
